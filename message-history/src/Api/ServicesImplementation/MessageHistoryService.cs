...
{
    public async Task<Message[]> FindMessagesForChat(int chatId)
    {
        var messages = await context.Messages.Where(message => message.ChatId == chatId).OrderByDescending(message => message.Id).ToArrayAsync();
        return messages;
    }
